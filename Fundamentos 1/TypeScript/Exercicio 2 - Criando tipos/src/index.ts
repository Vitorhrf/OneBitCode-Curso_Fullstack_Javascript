...
function savePlanet(nome: string, coordenadas: [number, number, number, number], situacao: Situation) {
    planets.push({
        nome,
        coordenadas,
        situacao,
...
function editSituationPlanet(nome: string, situacao: Situation) {
    try {
...
function findPlanet(nome: string): Planet | undefined {
...
function addSatellite(nome: string, satelite: string) {
    try {
        const planet = findPlanet(nome)
...
function removeSatellite(nome: string, satelite: string){
    try {
        const planet = findPlanet(nome)
...
function showPlanets(){
    let listPlanets: string = ''
    planets.forEach((planet) => {
        let listSatelites: string = ''
...
do{
    ...
    switch (opcao){
        case 1:
            ...
            const situacao = prompt("Digite a situação (habitado, habitavel, inabitavel, inexplorado):") as Situation
        ...
        case 2:
            ...
            if (novaSituacao && ["habitado", "habitavel", "inabitavel", "inexplorado"].includes(novaSituacao)) {
                editSituationPlanet(nome2, novaSituacao as Situation)
            } else {
