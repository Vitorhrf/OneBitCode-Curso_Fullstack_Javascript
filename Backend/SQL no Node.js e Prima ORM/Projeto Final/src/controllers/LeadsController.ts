...
import { LeadsRepository, LeadWhereParams } from "../repositories/LeadsController";
...
export class LeadsController {
    ...
    update: Handler = async (req, res, next) => {
        try {
            ...
            if (lead.status === "New" && body.status && body.status !== "Contacted") throw new HttpError(400, "um novo lead deve ser contatado antes de ter seu status atualizado para outros valores")
            ...
            // const updatedLead = await prisma.lead.update({ data: body, where: { id: Number(req.params.id) }})
