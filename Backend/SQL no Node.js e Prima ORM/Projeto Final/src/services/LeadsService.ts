...
export class LeadsService {
    ...
    async updateLead(leadId: number, params: Partial<CreateLeadAttributes>) {
        ...
        if (!lead) throw new HttpError(404, "lead não encontrado")
        if (lead.status === "New" && params.status && params.status !== "Contacted") throw new HttpError(400, "um novo lead deve ser contatado antes de ter seu status atualizado para outros valores")
